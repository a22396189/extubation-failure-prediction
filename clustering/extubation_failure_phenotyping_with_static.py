...
# 臨床剖析特徵（依分組）
PROFILE_FEATURES = {
    "Demographics": ["age", "BMI", "Charlson_Score"],
    "Vitals":       ["heart_rate", "resp_rate", "spo2", "mbp", "GCS"],
    "Gas/Vent":     ["PaO2", "PaCO2", "FiO2", "pH", "PEEP"],
    "Labs":         ["Cr", "WBC", "Hb", "Lactate"],
    "Interventions":["Vasopressor_use", "io_balance"],
}
...
def parse_args():
    ...
    p.add_argument("--use_static_in_cluster", type=int, default=0,
                   choices=[0, 1],
                   help="分群時是否加入靜態特徵（age/BMI/Charlson）：\n"
                        "  0 = 只用 Transformer Embedding（預設，推薦）\n"
                        "  1 = Embedding + Static Features（舊版行為）")
    return p.parse_args()
...
def extract_combined_features(model, dataloader, device,
                               pooling="last", use_static=False):
    """
    從 Transformer encoder 提取 patient embedding。

    pooling 選項：
      last  → 最後一個有效時間步的 hidden state  (B, d_model)
      mean  → 所有有效時間步的平均               (B, d_model)
      all   → 所有時間步 flatten concat          (B, T * d_model)  ← 後續需 PCA

    use_static：
      False → 只回傳 Transformer Embedding（推薦，用於可解釋性分群）
      True  → Embedding + Static Features concat（舊版行為）

    回傳：embeddings (N, dim)、labels (N,)、stay_ids (N,)、static_arr (N, stat_dim)
         ※ static_arr 不管 use_static 為何都會回傳，供後續 profiling 使用
    """
    model.eval()
    ...
    with torch.no_grad():
        for x_dyn, x_stat, y, step_present, sid in dataloader:
            ...
            if pooling == "last":
                ...
                valid_3d = valid.unsqueeze(-1)                              # (B, T, 1)
                pooled   = (z * valid_3d).sum(dim=1) / valid_3d.sum(dim=1).clamp(min=1e-6)
            else:  # "all"
                z_masked = z * valid.unsqueeze(-1)                          # (B, T, d_model)
    ...
    embeds_arr = np.vstack(all_embeds)   # (N, d_model) or (N, T*d_model)
    static_arr = np.vstack(all_stat)     # (N, stat_dim) — 永遠保留供 profiling 用

    if use_static:
        combined = np.hstack([embeds_arr, static_arr])
    else:
        combined = embeds_arr            # 只用 Transformer Embedding

    return combined, np.concatenate(all_labels), np.array(all_sids), static_arr
...
def plot_umap(features, cluster_labels, output_dir,
              seed=42, pooling_tag="last", use_static=False):
    print("\n[UMAP] 計算降維中...")
    ...
    color_map = {cid: palette[i] for i, cid in enumerate(sorted(np.unique(cluster_labels)))}
    colors    = [color_map[c] for c in cluster_labels]

    pooling_label = {"last": "Last-step", "mean": "Mean", "all": "All-steps (PCA)"}
    feat_tag  = "+ Static" if use_static else "only"
    subtitle  = f"Transformer Embedding ({pooling_label.get(pooling_tag, pooling_tag)}) {feat_tag}"
    fname_tag = f"{pooling_tag}_{'wStatic' if use_static else 'embOnly'}"

    fig, ax = plt.subplots(figsize=(9, 7))
    ax.scatter(coords[:, 0], coords[:, 1],
               c=colors, s=50, alpha=0.7, edgecolors="k", linewidths=0.3)
    patches = [mpatches.Patch(color=color_map[cid], label=f"Cluster {cid}")
               for cid in sorted(np.unique(cluster_labels))]
    ax.legend(handles=patches, fontsize=11, title="Cluster", title_fontsize=11)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"phenotype_umap_{fname_tag}.png"),
                dpi=300, bbox_inches="tight")
    plt.close()
...
def main():
    ...
    print(f"\n[Embedding] pooling={args.embed_pooling}  |  "
          f"use_static_in_cluster={use_static}")
    combined, orig_labels, sids, static_arr = extract_combined_features(
        model, loader, device,
        pooling=args.embed_pooling,
        use_static=use_static)
    mode_tag = ("Embedding + Static" if use_static
                else "Transformer Embedding only")
    print(f"[Features] 全 test set: {combined.shape[0]} 位，"
          f"分群維度={combined.shape[1]}  （{mode_tag}）")
